...
@section('content')
    ...
    @endif
    <form id="form" action="{{ route('events.update', ['event' => $event->id]) }}" method="POST">
        @method('PUT')
        @csrf
        <h1>Edit Event</h1>
        <div class=" rounded px-8 pt-6 pb-8 mb-4">
            <div class="mb-6">
                ...
                @enderror value="{{ old('name', $event->name) }}" name="name" id="event_name" type="text" placeholder="Event Name" required>
            </div>
            <div class="mb-6">
                ...
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror value="{{ old('description', $event->description) }}" name="description" id="event_description" type="text" placeholder="Event Description" required>
            </div>
            <div class="mb-6">
                ...
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror value="{{ old('date', $event->date) }}" name="date" id="event_date" type="date" required>
            </div>
            <div class="mb-6">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="event_date">
                    Event Time
            ...
            <div class="mb-6">
                ...
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror value="{{ old('location', $event->location) }}" name="location" id="event_location" type="text" placeholder="Event Location" required>
            </div>
            ...
            focus:outline-none focus:shadow-outline-purple">Edit Event
                </button>
            </div>
        </div>
    </form>
